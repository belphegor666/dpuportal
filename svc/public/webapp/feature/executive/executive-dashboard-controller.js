...
angular.module('app').controller("executiveDashboardController", ["$scope", "$state", "$log", "repository", function ($scope, $state, $log, repository) {

    // Constant - for trace and debug
    $scope.controllerName = "executiveDashboardController";
    ...
    function getProjects() {
        repository.getProjects().then(function (results) {
            ...
            if (results.length == 0) {
                ...
                $scope.noProjectsFound = false;
                //filter to not show projects with draft status
                $scope.search = {status: '!' + $$refdata.PROJECT_STATUS.DRAFT};
            }
            ...
            results.forEach(function (project) {
                ...
                if (project.status == $$refdata.PROJECT_STATUS.CLOSED_OK) {
                    ...
                    if (project.costSavings != null) {
                        $scope.completedCost += project.costSavings;
                    }
                }
            });
    ...
    var filterProjects = function (projects, filterStatus) {
        ...
        return projects.filter(function (project) {
            ...
            if (filterStatus == $$refdata.PROJECT_STATUS.DRAFT) {
                ...
                if ($scope.isAdmin) {
                    //draft projects only visible to Admin user
                    $scope.search = '';
